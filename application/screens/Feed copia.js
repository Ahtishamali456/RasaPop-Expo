...
export default function Feed(props) {
  ...
  const onUnlikeItem = (UserId, itemId) => {
    let data = likes.filter((item) => item !== itemId);

    setLikes(data);
    submitUnLike(UserId, itemId);
  };

  const onLikeItem = (UserId, itemId) => {
    ...
    if (likes.includes(item.id)) {
      return (
        <Button
          ...
          uppercase={false}
          onPress={() => onUnlikeItem(userInfo.user_id, item.id)}
        >
      ...
      if (item.isliked) {
        ...
        return (
          <Button
            ...
            labelStyle={Styles.feedActionLabel}
            uppercase={false}
            onPress={() => onLikeItem(userInfo.user_id, item.id)}
          >
            {!likes.includes(item.id) ? item.total_likes : item.total_likes - 1}
          </Button>
        );
      }
    }
        ...
        <View>
          {map(items, (item, index) => (
            <Card
              key={index}
              mode="outlined"
              style={{ borderColor: "#eee", marginBottom: 20 }}
            >
              <Card.Title
                title={item.author}
                ...
                left={(props) => (
                  <TouchableOpacity
                    activeOpacity={1}
                    onPress={() => onClickMember(item.author_id, item.author)}
                  >
                    <Avatar.Image
                      {...props}
                      size={45}
              ...
              <TouchableOpacity
                activeOpacity={0.9}
                onPress={() => onClickRecipe(item.id, item.title)}
              >
              ...
              >
                ...
                <Button
                  ...
                  mode="text"
                  color="#959595"
                  onPress={() => onClickComments(item.id)}
                  labelStyle={Styles.feedActionLabel}
                  uppercase={false}
                >
                  {item.total_comments}
